...
def generate_launch_description():
    ...
    lidar_topic_name_arg = DeclareLaunchArgument(
        "lidar_topic_name", description="Topic name to listen for PointCloud2 input from the LiDAR (for example '/livox/lidar')")
    ...
    mola_lo_pipeline_arg = DeclareLaunchArgument(
        "mola_lo_pipeline", default_value="../pipelines/lidar3d-katana.yaml", description="The LiDAR-Odometry pipeline configuration YAML file defining the LO system. Absolute path, or relative to 'mola-cli-launchs/lidar_odometry_ros2.yaml'. Default is the 'lidar3d-default.yaml' system described in the IJRR 2025 paper.")
    ...
    # ~~~~~~~~~~~~
    generate_lazy_load_arg = DeclareLaunchArgument(
    	"generate_lazy_load_pcd", default_value="True", description="Whether to export individual PCD files alongside the simplemap")
    generate_lazy_load_env_var = SetEnvironmentVariable(
    	name='MOLA_SIMPLEMAP_GENERATE_LAZY_LOAD', value=LaunchConfiguration('generate_lazy_load_pcd'))
    # ~~~~~~~~~~~~
    ...
    mola_footprint_to_base_link_tf_arg = DeclareLaunchArgument(
        "mola_footprint_to_base_link_tf", default_value="[0.0, 0, -0.0, 0, 0, 0]", description="Can be used to define a custom transformation between base_footprint and base_link. The coordinates are [x, y, z, yaw_deg, pitch_deg, roll_deg].")
    ...
    enforce_planar_motion_arg = DeclareLaunchArgument(
        "enforce_planar_motion", default_value="True", description="Whether to enforce z, pitch, and roll to be zero.")
    ...
    forward_ros_tf_odom_to_mola_arg = DeclareLaunchArgument(
        "forward_ros_tf_odom_to_mola", default_value="True", description="Whether to import an existing /tf 'odom'->'base_link' odometry into the MOLA subsystem.")
    ...
    mola_precise_deskew_from_imu_arg = DeclareLaunchArgument(
        "mola_precise_deskew_from_imu", default_value="True", description="Whether to use the IMU to deskew the LiDAR scans more precisely")
    ...
    return LaunchDescription([
        ...
        generate_lazy_load_arg,      # MOD
    	generate_lazy_load_env_var,  # MOD
